...
export default function ChatWidget() {
  ...
  return (
    <div className="w-full max-w-xl mx-auto">
      <div className="rounded-2xl border border-[#2dd4bf]/20 bg-[#0d1526] overflow-hidden">
        {/* Terminal header */}
        <div className="flex items-center gap-2 px-4 py-3 border-b border-white/5 bg-white/[0.02]">
          <span className="w-3 h-3 rounded-full bg-red-500/60" />
          <span className="w-3 h-3 rounded-full bg-yellow-500/60" />
          <span className="w-3 h-3 rounded-full bg-green-500/60" />
          <span className="ml-3 text-xs text-[#f0ede8]/30 font-mono">christian-schou-ai ~ ask anything</span>
        </div>

        {/* Messages */}
        <div className="p-4 h-72 overflow-y-auto flex flex-col gap-3 scrollbar-thin">
        ...
        <div className="border-t border-white/5 p-3 flex gap-2">
          <input
            ...
            className="flex-1 bg-transparent text-sm text-[#f0ede8] placeholder-[#f0ede8]/25 outline-none px-2"
          />
          <button
            onClick={send}
            disabled={loading || !input.trim()}
            className="px-4 py-2 rounded-xl bg-[#2dd4bf] text-[#0a0f1e] text-sm font-semibold disabled:opacity-40 hover:bg-[#2dd4bf]/90 transition-all duration-200"
          >
            Send
          </button>
        </div>
